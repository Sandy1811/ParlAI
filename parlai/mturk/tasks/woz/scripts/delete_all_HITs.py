...
def expire_HIT_group(group_to_hit, task_group_id, sandbox):
    hits_expired = 0
    for hit_id, status in group_to_hit[task_group_id].items():
        ...
        #     print(f"Could not approve {hit_id}")
...
def main():
    ...
    for task_group_id in task_group_ids:
        if input(f"Confirm deletion of task group {task_group_id} with {len(group_to_hit[task_group_id])} HITs [y/n]").lower() != "y":
            continue
        expire_HIT_group(group_to_hit, task_group_id, sandbox)
